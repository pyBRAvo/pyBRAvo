...
def gen_IDs_values_constraint(chunks, variable_name):
    """
    Generation of a SPARQL VALUES clause to match entities by IDs.
        VALUES ?id {<uniprotID> <>}
    """
    filter_clause = ''
    if len(chunks) > 0 :
...
def upstream_regulation(to_be_explored, already_explored = [], sif_network = [], current_depth = 0, explored_reg = 0):
    ...
    for regulators in chunks :
        ...
        query = Template(tpl_select_reg_query_IDs)
        ...
        values_constraint = util.gen_IDs_values_constraint(chunks=regulators, variable_name='?id')

        q = query.substitute(
                    inject_values = values_constraint)
        ...
        for result in results["results"]["bindings"]:
            ...
            if "controlType" in result.keys():
                source, source_id, reg_type, target, provenance = result["controllerName"]["value"], result["controller_id"]["value"], \
                                                  result["controlType"]["value"], result["controlledName"]["value"], result["source"]["value"]
            else:
                source, source_id, target, provenance = result["controllerName"]["value"], result["controller_id"]["value"], \
            ...
            if source_id not in already_explored:
                if source_id not in to_be_explored:
                    to_be_explored.append(source_id)
                    explored_reg += 1
